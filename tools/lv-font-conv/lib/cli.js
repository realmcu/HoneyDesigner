...
`
  ...
  parser.add_argument('--extract-glyph-bitmap', {
    ...
    help: 'Extract glyph bitmap data to a separate binary file for flash storage.\n' +
          'Note: Only supported with --format lvgl.'
  });
  ...
  // Extract glyph bitmap is only supported with LVGL format
  if (args.extract_glyph_bitmap && args.format !== 'lvgl') {
    parser.error('--extract-glyph-bitmap is only supported with --format lvgl');
  }
