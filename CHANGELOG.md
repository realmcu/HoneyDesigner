...
- 新增项目级多语言（i18n）系统：多语言目录（catalog）、标签绑定与翻译、设计器预览语言切换，以及多语言管理器弹窗（表格编辑、搜索、Key 重命名/删除、语言增删与可搜索语言选择器）
- 代码生成与字体转换接入多语言：LVGL/HoneyGUI 标签生成使用默认语言翻译文本；字体转换自动收集项目目录多语言字符集
- 新增「Copy for AI」右键操作：保存当前文件、按缩放/平移截取画布截图（含选中高亮），并组装设计描述文本一并写入剪贴板，供 AI 编码代理使用
- stream 组件新增 h264 类型

### Fixed

- 修复 hg_view 尺寸未随项目分辨率同步的问题：重新打开文件后仍保留旧尺寸
- 修复多段 LVGL timeline 动画仅播放最后一段的问题：禁用 early_apply 避免后段动画在前段播放期间抢先应用起始值（#18）
- 修复图片转换配置未转发 dither 开关的问题，导致 Floyd-Steinberg 抖动始终未生效
- 修复切换选中组件后选中框残留在旧组件上、不跟随移动的问题
- 修复资源管理器用上下键浏览 HML 时预览未自动打开、焦点丢失的问题
- 修复数字输入框保留多余前导零（如 050）的显示问题（#19）

### Internal

- 同步仿真库（fix vector font thai language blend）
- 补齐 VSIX 打包运行时依赖白名单，排除本地缓存日志
